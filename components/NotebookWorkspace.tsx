...
import { GraduationCap, ArrowRight, Activity, Palette, Sparkles, Plus, Search, Trash2, Save, FilePlus, MessageSquare, ArrowLeft } from 'lucide-react';
import { Paper, Message, ScholarProfile, AppState, Notebook } from '../types';
...
interface NotebookWorkspaceProps {
  notebook: Notebook;
  onSave: (updatedNotebook: Notebook) => void;
  onBack: () => void;
}

const NotebookWorkspace: React.FC<NotebookWorkspaceProps> = ({ notebook, onSave, onBack }) => {
  // Initialize state from props
  const [profile, setProfile] = useState<ScholarProfile | null>(notebook.profile);
  const [chatMessages, setChatMessages] = useState<Message[]>(notebook.chatMessages);
  const [currentTheme, setCurrentTheme] = useState<ThemeName>((notebook.theme as ThemeName) || 'Ocean');

  const [scholarName, setScholarName] = useState('');
  const [manualPaperTitle, setManualPaperTitle] = useState('');
  const [appState, setAppState] = useState<AppState>(notebook.profile ? AppState.READY : AppState.IDLE);
  ...
  useEffect(() => {
    onSave({
      ...
      theme: currentTheme,
      updatedAt: Date.now(),
      title: profile?.name || notebook.title // Update title if profile name changes
    });
  }, [profile, chatMessages, currentTheme]);
  ...
  const isAnyProcessing = useMemo(() => {
    return profile?.papers.some(p => p.status === 'downloading' || p.status === 'processing') || false;
  }, [profile?.papers]);

  // Clear all data (local to this notebook)
  const handleClearSession = () => {
    if (window.confirm("Are you sure you want to clear this notebook's data? This cannot be undone.")) {
      setProfile(null);
      setChatMessages([]);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    // Note: We deliberately replace the profile here for a new search
    setProfile(null);
    setChatMessages([]);
    setSelectedPaperIds(new Set());
  ...
  const handleManualAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      setAppState(profile ? AppState.READY : AppState.IDLE);
  ...
  const handleSelectAll = () => {
    if (!profile) return;
    const allIds = profile.papers.map(p => p.id);
  ...
  const handleGenerateSelected = () => {
    if (!profile) return;
    const papersToProcess = profile.papers.filter(p => selectedPaperIds.has(p.id));
    ...
    if (analyzeMatch) {
        ...
        try {
            ...
            if (newPaper) {
                ...
                let added = false;
                setProfile(prev => {
                    const newProfile = prev || { name: 'Custom Search', papers: [] };
                    if (newProfile.papers.some(p => p.title.toLowerCase() === newPaper.title.toLowerCase())) {
                         return newProfile; 
                    }
                    added = true;
                    return {
                        ...newProfile,
                        papers: [newPaper, ...newProfile.papers]
                    };
                });
        ...
        <div className="glass-panel p-6 md:p-8 border-b border-white/40 shadow-sm z-20">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div className="bg-gradient-to-br from-scholarly-500 to-scholarly-700 p-2.5 rounded-xl shadow-lg shadow-scholarly-200/50">
                <span className="text-xl leading-none flex items-center justify-center h-6 w-6">{notebook.emoji}</span>
              </div>
              <div>
                 <h1 className="text-xl font-serif font-bold text-slate-900 tracking-tight leading-none">{notebook.title}</h1>
                 <p className="text-xs text-slate-500 font-medium mt-1">Edited {new Date(notebook.updatedAt).toLocaleTimeString()}</p>
              </div>
            </div>
          ...
          {profile && (
            <div className="mt-6 flex flex-wrap gap-3 animate-in fade-in slide-in-from-top-4 duration-500">
               <div className="bg-white/60 backdrop-blur-sm px-4 py-2 rounded-lg border border-white/50 shadow-sm text-sm font-semibold text-slate-700 flex items-center gap-2">
                  <span className="text-xl">🏛️</span> {profile.affiliation || "Library Collection"}
               </div>
